  ...
  mkdir -p docker-logs && for dir in logs/*/; do name=$(basename "$dir"); docker logs "quant-${name}" > "docker-logs/${name}.log" 2>&1 || echo "Container quant-${name} not found"; done && tar -czvf docker-logs.tar.gz docker-logs;git pull;git add .;git commit -m 'x';git push


  ./start-all-strategies.sh down;rm -rf docker-logs/ docker-logs.tar.gz;sudo rm -rf logs/ data/;git pull;git add .;git commit -m 'x';git push;docker build -t quant-trading-system:latest --no-cache .;./start-all-strategies.sh up;sleep 60 && mkdir -p docker-logs && for dir in logs/*/; do name=$(basename "$dir"); docker logs "quant-${name}" > "docker-logs/${name}.log" 2>&1 || echo "Container quant-${name} not found"; done && tar -czvf docker-logs.tar.gz docker-logs;git pull;git add .;git commit -m 'x';git push;
  ...
  # 自定义完整配置
STRATEGY_NAME=MACD DEFAULT_SYMBOLS="SOL/USDT" DEFAULT_TIMEFRAME=4h \
    docker-compose -f docker-compose.single-strategy.yml up -d
