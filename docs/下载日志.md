● 有两种方式：
  ...
  git pull;rm -rf docker-logs/ docker-logs.tar.gz;sudo rm -rf logs/ data/;mkdir -p docker-logs && for dir in logs/*/; do name=$(basename "$dir"); docker logs "quant-${name}" > "docker-logs/${name}.log" 2>&1 || echo "Container quant-${name} not found"; done && tar -czvf docker-logs.tar.gz docker-logs;git add .;git commit -m 'x';git push;
