  ...
  ./start-all-strategies.sh down;./scripts/start-with-shared-market.sh stop;git pull;rm -rf docker-logs/ docker-logs.tar.gz;sudo rm -rf logs/ data/;git add .;git commit -m 'x';git push;./start-all-strategies.sh up;./scripts/start-with-shared-market.sh multi;mkdir -p docker-logs && for dir in logs/*/; do name=$(basename "$dir"); docker logs "quant-${name}" > "docker-logs/${name}.log" 2>&1 || echo "Container quant-${name} not found"; done && tar -czvf docker-logs.tar.gz docker-logs;git add .;git commit -m 'x';git push;
