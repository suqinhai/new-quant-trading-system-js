...
export class KrakenExchange extends BaseExchange {
  ...
  _createExchange() {
    ...
    const isFutures = ['swap', 'future'].includes(this.config.defaultType);
    const ExchangeClass = isFutures ? ccxt.krakenfutures : ccxt.kraken;
  ...
  async setLeverage(leverage, symbol) {
    ...
    if (this.config.defaultType === 'spot') {
      ...
      throw this._createError('UNSUPPORTED', '杠杆仅适用于合约交易 / Leverage only available for futures');
    }
    ...
    return this._executeWithRetry(async () => {
      try {
        ...
        if (error.message && (error.message.includes('same') || error.message.includes('not modified'))) {
          console.log(`[${this.name}] ✓ 杠杆无需更改 / Leverage already set`);
          return { success: true, message: 'Leverage already set' };
        }
  ...
  async fetchTradingVolume() {
    ...
    return this._executeWithRetry(async () => {
      ...
      const response = await this.exchange.privatePostTradeVolume();

      // 返回格式化的交易量信息 / Return formatted trade volume info
      return {
        currency: response.result?.currency || 'USD',
        volume: parseFloat(response.result?.volume || 0),
        fees: response.result?.fees || {},
        feesMaker: response.result?.fees_maker || {},
        exchange: this.name,
        timestamp: Date.now(),
      };
    }, '获取交易量等级 / Fetch trading volume tier');
  }
  ...
  async fetchRecentTrades(symbol, limit = 100) {
    ...
    }, `获取最近交易 / Fetch recent trades: ${symbol}`);
  }
    ...
    return this._executeWithRetry(async () => {
      ...
      return positions
        .filter(pos => {
          ...
          const size = Math.abs(parseFloat(pos.contracts || 0));
          return size > 0;
        })
        .map(pos => ({
          // 交易对 / Symbol
          ...
          // 保证金 / Collateral
          collateral: parseFloat(pos.collateral || 0),
  ...
  async fetchDepositAddress(currency, params = {}) {
    ...
    return this._executeWithRetry(async () => {
      ...
      const address = await this.exchange.fetchDepositAddress(currency, params);
    ...
    }, `获取存款地址 / Fetch deposit address: ${currency}`);
  }
   ...
   */
  async fetchWithdrawals(currency = undefined, since = undefined, limit = 50) {
    // 确保已连接 / Ensure connected
    this._ensureConnected();
    ...
    return this._executeWithRetry(async () => {
      ...
      const withdrawals = await this.exchange.fetchWithdrawals(currency, since, limit);
    ...
    }, `获取提款记录 / Fetch withdrawals: ${currency || 'all'}`);
  }
   ...
   */
  async fetchDeposits(currency = undefined, since = undefined, limit = 50) {
    // 确保已连接 / Ensure connected
    this._ensureConnected();
    ...
    return this._executeWithRetry(async () => {
      ...
      const deposits = await this.exchange.fetchDeposits(currency, since, limit);
    ...
    }, `获取存款记录 / Fetch deposits: ${currency || 'all'}`);
  }
}
