 ...
 *
 * 继承自 BaseExchange，实现 Gate.io 特定的功能
 * Extends BaseExchange to implement Gate.io-specific functionality
 */
...
export class GateExchange extends BaseExchange {
  ...
  _createExchange() {
    ...
    if (this.config.sandbox) {
      // Gate.io 测试网配置 / Gate.io testnet configuration
    ...
    // Gate.io 在 CCXT 中使用 'gate' 类 / Gate.io uses 'gate' class in CCXT
    return new ccxt.gate(exchangeConfig);
  }

  // ============================================
  // Gate.io 特有方法 / Gate.io-Specific Methods
  // ============================================
  ...
  async fetchServerTime() {
    ...
    return this._executeWithRetry(async () => {
      ...
      const response = await this.exchange.publicSpotGetTime();

      // 返回服务器时间戳 (毫秒) / Return server timestamp (milliseconds)
      return response.server_time * 1000;
    }, '获取服务器时间 / Fetch server time');
  }
  ...
  async setPositionMode(hedgeMode = false) {
    ...
    return this._executeWithRetry(async () => {
      try {
        // Gate.io 使用 dual_mode 参数 / Gate.io uses dual_mode parameter
        // true = 双向持仓 / Dual position mode (hedge)
        // false = 单向持仓 / Single position mode (one-way)
        const result = await this.exchange.setPositionMode(hedgeMode);
        ...
        if (error.message && (error.message.includes('No need to change') || error.message.includes('same'))) {
  ...
  async createStopOrder(symbol, side, amount, options = {}) {
    ...
    console.log(`[${this.name}] 创建止损订单 / Creating stop order:`, {
      ...
      stopType: options.stopType || 'stop_loss',
    });
    ...
    return this._executeWithRetry(async () => {
      ...
      const orderType = options.stopType === 'take_profit' ? 'takeProfit' : 'stopLoss';
   ...
   */
  async fetchIncomeHistory(symbol = undefined, limit = 100) {
    // 确保已连接 / Ensure connected
    this._ensureConnected();
    ...
    return this._executeWithRetry(async () => {
      ...
      if (symbol) {
        params.contract = this.exchange.marketId(symbol);
      }

      // 调用 Gate.io API 获取收入历史 / Call Gate.io API to fetch income history
      const response = await this.exchange.privateFuturesGetSettleAccountBook(params);

      // 返回格式化的收入历史 / Return formatted income history
      return response.map(item => ({
        symbol: item.contract || null,
        incomeType: item.type,
        income: parseFloat(item.change),
        asset: item.text || 'USDT',
