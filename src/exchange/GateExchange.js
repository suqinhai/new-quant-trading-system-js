 ...
 */
export class GateExchange extends BaseExchange { // 导出类 GateExchange
  /**
   * 构造函数
   * Constructor
  ...
  _createExchange() { // 调用 _createExchange
    ...
    return new ccxt.gate(exchangeConfig); // 返回结果
  } // 结束代码块
  ...
  async fetchServerTime() { // 执行语句
    ...
    return this._executeWithRetry(async () => { // 返回结果
      ...
      const response = await this.exchange.publicSpotGetTime(); // 定义常量 response

      // 返回服务器时间戳 (毫秒) / Return server timestamp (milliseconds)
      return response.server_time * 1000; // 返回结果
  ...
  async setPositionMode(hedgeMode = false) { // 执行语句
    ...
    return this._executeWithRetry(async () => { // 返回结果
      try { // 尝试执行
        ...
        if (error.message && (error.message.includes('No need to change') || error.message.includes('same'))) { // 条件判断 error.message && (error.message.includes('No ...
   ...
   */
  async fetchIncomeHistory(symbol = undefined, limit = 100) { // 执行语句
    // 确保已连接 / Ensure connected
    this._ensureConnected(); // 调用 _ensureConnected
    ...
    return this._executeWithRetry(async () => { // 返回结果
      ...
      if (symbol) { // 条件判断 symbol
        params.contract = this.exchange.marketId(symbol); // 赋值 params.contract
      } // 结束代码块

      // 调用 Gate.io API 获取收入历史 / Call Gate.io API to fetch income history
      const response = await this.exchange.privateFuturesGetSettleAccountBook(params); // 定义常量 response
...
// 默认导出 / Default export
export default GateExchange; // 默认导出
