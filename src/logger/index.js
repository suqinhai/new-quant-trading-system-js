 ...
 */
function createLoggerModule(config = {}) {
  // 创建 PnL 日志记录器实例 / Create PnL Logger instance
  const pnlLogger = new PnLLogger(config.pnlLogger || {});
  ...
  const alertManager = new AlertManager(config.alertManager || {});

  // 创建指标导出器实例 / Create Metrics Exporter instance
  const metricsExporter = new MetricsExporter(config.metricsExporter || {});

  // 连接警报管理器和通知器 / Connect Alert Manager to Notifiers
  alertManager.setNotifiers({
  ...
  return {
    ...
    setDataSources(sources) {
      ...
      pnlLogger.setDataSources(sources);

      // 设置到 Telegram 通知器 / Set to Telegram Notifier
      telegramNotifier.setDataSources(sources);

      // 设置到警报管理器 / Set to Alert Manager
      alertManager.setDataSources(sources);

      // 设置到指标导出器 / Set to Metrics Exporter
      metricsExporter.setDataSources(sources);
    },
    ...
    async startAll() {
      ...
      console.log('[Logger] 所有日志服务已启动 / All logging services started');
    },
    ...
    async stopAll() {
      ...
      console.log('[Logger] 所有日志服务已停止 / All logging services stopped');
    },
    ...
    triggerAlert(alertConfig) {
      ...
      return alertManager.triggerAlert(alertConfig);
    },
  };
}
