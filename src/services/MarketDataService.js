 ...
 */
function getConfiguredExchanges(config) { // 定义函数 getConfiguredExchanges
...
function buildDefaultConfig() { // 定义函数 buildDefaultConfig
  const systemConfig = loadConfig(); // 定义常量 systemConfig
  ...
  return { // 返回结果
    ...
    exchanges: process.env.MARKET_DATA_EXCHANGES // Exchange list (prefer env var, otherwise dynamically get configured exchanges)
      ...
      : getConfiguredExchanges(systemConfig), // 执行语句

    // 交易所配置映射 / Exchange config map
    exchangeConfigs: systemConfig.exchange || {}, // 交易所配置映射

    // 交易类型 (swap = 永续合约) / Trading type (swap = perpetual)
    tradingType: process.env.TRADING_TYPE || 'swap', // 交易类型 (swap = 永续合约)

    // Cache configuration
    cache: systemConfig.marketData?.cache || {}, // Cache configuration
    // Kline timeframe (e.g. 5m, 1h)
    klineTimeframe: process.env.MARKET_DATA_TIMEFRAME || systemConfig.marketData?.klineTimeframe || '1h',
...
export class MarketDataService extends EventEmitter { // 导出类 MarketDataService
  ...
  constructor(config = {}) { // 构造函数
    super(); // 调用父类
    const defaultConfig = buildDefaultConfig(); // 定义常量 defaultConfig

    // 合并配置 / Merge configuration
    this.config = { // 设置 config
      redis: { ...defaultConfig.redis, ...config.redis }, // redis
      exchanges: config.exchanges || defaultConfig.exchanges, // 交易所
      exchangeConfigs: config.exchangeConfigs || defaultConfig.exchangeConfigs, // 交易所配置
      tradingType: config.tradingType || defaultConfig.tradingType, // 交易类型
      cache: { ...defaultConfig.cache, ...config.cache }, // cache
      klineTimeframe: config.klineTimeframe || defaultConfig.klineTimeframe, // kline timeframe
      heartbeatInterval: config.heartbeatInterval || defaultConfig.heartbeatInterval, // heartbeat间隔
      subscribeAll: config.subscribeAll ?? defaultConfig.subscribeAll, // subscribeAll
      symbols: config.symbols || defaultConfig.symbols, // 交易对列表
    }; // 结束代码块
